...
| `reusable-release.yml`                | Build → test built artifact → deploy to wp.org   | `plugin-slug`, `project-path`, `php-version`, `requires-scoped-dependencies`, `plugin-check`, `generate-pot` (no secrets) |
